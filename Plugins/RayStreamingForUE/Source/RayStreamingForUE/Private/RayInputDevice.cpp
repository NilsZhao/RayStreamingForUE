...
DECLARE_LOG_CATEGORY_EXTERN(RayStreamerInputDevice, Log, VeryVerbose);

DEFINE_LOG_CATEGORY(RayStreamerInputDevice);

DECLARE_LOG_CATEGORY_EXTERN(RayStreamerInput, Log, VeryVerbose);

DEFINE_LOG_CATEGORY(RayStreamerInput);
...
void FRayInputDevice::ParseEventMessage(const FString& InputMessage)
{
	...
	else if (Head == "RayInput_MouseKey")
	{
		...
		FVector2D CursorPos = GEngine->GameViewport->GetWindow()->GetPositionInScreen() + FVector2D(
			MouseKeyInput.PositionX, MouseKeyInput.PositionY);
		RayStreamerApplicationWrapper->Cursor->SetPosition(CursorPos.X, CursorPos.Y);
		*/
